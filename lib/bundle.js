...
function bundle (parser, options) {
  ...
  debug('Remapping $ref pointers in %s', parser.$refs._root$Ref.path);
  remap(inventory,parser.$refs,parser);
}
...
function crawl (parent, key, path, pathFromRoot, inventory, $refs, options) {
  ...
  debug("crawler: key=%s, path=%s, pathFromRoot=%s,obj=%s",key,path,pathFromRoot,JSON.stringify(obj,null,'\t'));
  if (obj && typeof obj === 'object') {
    if ($Ref.isAllowed$Ref(obj)) {
      debug("crawler: inventory$Ref(1)");
      ...
      keys.forEach(function (key) {
        debug("crawler: definitions[%s]",key);
        var keyPath = Pointer.join(path, key);
        var keyPathFromRoot = Pointer.join(pathFromRoot, key);
        var value = obj[key];

        if ($Ref.isAllowed$Ref(value)) {
          debug("crawler: isAllowed$Ref(%s)",JSON.stringify(value,null,'\t'));
...
function inventory$Ref ($refParent, $refKey, path, pathFromRoot, inventory, $refs, options) {
  ...
  debug("inventory$Ref:$refParent=%s",JSON.stringify($refParent));
  debug("inventory$Ref:$ref=%s",JSON.stringify($ref));
  debug("inventory$Ref:before $refPath $ref.$ref=%s",JSON.stringify($ref.$ref));
  var $refPath = url.resolve(path, $ref.$ref);
  var pointer = $refs._resolve($refPath, options);
  debug("inventory$Ref:file=%s",url.stripHash(pointer.path))
  var depth = Pointer.parse(pathFromRoot).length;
  var file = url.stripHash(pointer.path);
  var hash = url.getHash(pointer.path);
  debug("inventory$Ref:root %s",$refs._root$Ref.path);
  ...
  debug("inventory$Ref:%s",JSON.stringify(inventory,null,'\t'))
  debug("Recursive crawl: pointer.value=%s, pointer.path=%s pathFromRoot=%s",JSON.stringify(pointer.value),pointer.path,pathFromRoot);
 ...
 */
function remap (inventory,$refs,parser) {
  // Move all external refs to local ones
  inventory.forEach(function (i) {
    if (i.external) {
	// Add external reference to root schema definitions
	// TODO: 
	// - "definitions" key should be configurable as any user-defined pointer
	// - bug if several definitions conflict in the reference tree

	//debug('BEFORE')
	//debug(JSON.stringify(i,null,'\t'))

    var key = i.hash.substring(i.hash.lastIndexOf('/')+1);					// TODO: we cut the path, possibly leaving conflicts
	var rootDefinitions = '#/definitions'							// TODO: make this configurable
	parser.schema.definitions[key] = $Ref.dereference(i.$ref, i.value);

	i.hash = Pointer.join(rootDefinitions,key)
  	i.file = $refs._root$Ref.path;			// Declare reference in root schema
  	i.value =  { $ref: Pointer.join(rootDefinitions,key) }					// e.g. "$ref": "#/definitions/key
	i.depth = 1
	i.extended = $Ref.isExtended$Ref(i.$ref);
	i.external = false
	i.circular = false
	//debug('AFTER')
	//debug(JSON.stringify(i,null,'\t'))
